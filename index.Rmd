...
```{r EDA plots, fig.width=7.25, fig.height=4.5, echo=FALSE, cache=TRUE, warning=FALSE}
...
e.logDeliveryHist <- ggplot(data=reports[!is.na(reports$ReportDeliveryTime),], aes(x=log(ReportDeliveryTime))) +
        geom_histogram(fill="orange", binwidth = .2) + labs(title="Log Report Delivery Time", x="x=log(Report Delivery Time)")
...
```{r EDA plots2, fig.width=7.25, fig.height=7.25, cache=TRUE, echo=FALSE, warning=FALSE}
...
The coefficients and a 95% confidence interval are included in the tables below:

```{r Obj1 Final Model Selection, cache=FALSE, echo=FALSE, warning=FALSE, fig.height=9}
...
# Format in two tables:
as_image(kable_styling(kable(head(finalDF, 28))), file = './img/final1.png')
as_image(kable_styling(kable(tail(finalDF, 25))), file = './img/final2.png')
...
## Explanatory Data Analysis of Server and HourBinned Variables
...
```{r FilterAndBin, cache=FALSE, echo=FALSE}
...
```{r continuesummary, cache=TRUE, echo=FALSE}
...
```{r numReportPlot, cache=TRUE, echo=FALSE, fig.width=7.25, fig.align='center'}
...
```{r ANOVAIIINoLog, cache=TRUE, echo=FALSE, fig.width=7.25, fig.align='center', fig.height=2.5}
...
```{r plotbySrvHour, cache=TRUE, echo=FALSE, fig.width=7.25, fig.align='center'}
...
```{r loggedResponseResidual, cache=TRUE, echo=FALSE, fig.width=7.25, fig.align='center', fig.height=3}
...
grid.arrange(plot1.fltr.log, plot2.fltr.log, plot3.fltr.log, nrow=1, ncol=3)
...
Due to the residual and Q-Q plot of the untransformed data we deemed it necessary to perform a log transform of the response data. The data responded well to the transformation displaying all the characteristics of normalized data.
...
## Final Thoughts
Special thanks to Gilsbar for sharing this data for our project. It added a layer of complexity and realism to use company data to solve an actual problem in a production environment. The report prediction has always been a great idea and viewed as a 'nice to have,' but to date, no one has dedicated time to the problem until now.  We faced a lot of early challenges in just pulling data out of the production environment in a way that would allow analysis, especially by several widely dispersed individuals. Extracting, anonymizing, tidying all took very many hours of work before we could even begin initial feature selection.  Now that we have established an initial analysis, it is something Gilsbar can further refine, and would be interesting to apply more advanced techniques to the data to see just how accurate we can make the predictions.
...
The correlation between the sample size and variance is -0.81. The coefficient of sample size variation $\Delta n$ was calculated to be: $\frac{SD(Group Sample Size)}{mean(Group Sample Size)}=0.932$
...
| |	**−1** |	**0.16; 0.33; 0.50**|	**Liberal**[^1]|


[^1]: The **Bold** line in the table was utilized for p-value adjustments. 


# Appendix: MLR Performance Testing

```{r Performance testing, cache=TRUE, echo=FALSE, fig.height=3, fig.width=7.25, fig.align='center'}
...
plot(predictions, ytest, xlim = c(5,10), ylim = c(5,10), main="Predicted vs Test")
