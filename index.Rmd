...
reports$NumDays <- ifelse(is.na(reports$NumDays),.1,reports$NumDays)
reports$GroupCount <- ifelse(is.na(reports$GroupCount),.1,reports$GroupCount)
reports$GroupCount <- ifelse((reports$GroupCount == 0),.1,reports$GroupCount)
...
fit <- lm(log(ReportBuildTime)~Server + Priority +log(GroupCount)
          +ReportCategory+log(NumDays)+DayOfWeek+DayOfMonth+AgentCount+RptFrmt+DelivMthd+HourOfDay, data=reports)
summary(fit)

fit <-lm(log(ReportBuildTime)~ReportCategory+TimeBlock+ReportCategory*TimeBlock, data=reports)
summary(fit)


fit2 <-lm(log(ReportBuildTime)~ReportCategory+TimeBlock+DayOfMonth+
            ReportCategory*TimeBlock+DayOfMonth*TimeBlock + ReportCategory*DayOfMonth+
            ReportCategory*TimeBlock*DayOfMonth, data=reports2)
summary(fit2)

reports$first <- ifelse(reports$DayOfMonth == "01", 1, 0)
reports$six <- ifelse(reports$HourOfDay == "06", 1, 0)

fit3 <- lm(log(ReportBuildTime)~ReportCategory+first+six+first*six+ReportCategory*six+ReportCategory*firs, data=reports)
summary(fit3)
...
summary(reports$ReportDeliveryTime)

#Drop two outliers, reports which are highly likely to be errors:
reports <- reports %>% filter(ReportDeliveryTime < 400000)
reports <- reports %>% filter(ReportDeliveryTime > 0)
