...
First we did a fit with all parameters and get a resulted $R^{2}$ of 0.54, which is not too bad.

```{r Obj1 Sample Selection, cache=TRUE, include=FALSE}
...
### Model Selection
#### Forward selection selected coefficients:  

SchedFreqOneTime, SchedFreqWeek, ReportCategoryClaims, DayOfMonth04, HourOfDay21 , HourOfDay22, AgentCount, ReportCategoryEligibility, HourOfDay02, HourOfDay03 

```{r Obj1 Forward Selection, cache=TRUE, echo=FALSE, warning=FALSE, fig.height=3}
...
#### Sequential Repetition selected coefficients:

SchedFreqOneTime, SchedFreqWeek, DayOfMonth02, HourOfDay21, HourOfDay22, AgentCount, ReportCategoryEligibility, ReportCategoryMiscellaneous, HourOfDay02, HourOfDay03

```{r Obj1 Seq Rep Selection, cache=TRUE, echo=FALSE, warning=FALSE, fig.height=3}
...
#### Final Model Adjusted $R^{2}$ and Selected coefficients:

$R^{2}$ = .52

SchedFreqOneTime, SchedFreqWeek, HourOfDay07,  HourOfDay08,  HourOfDay09, HourOfDay10,  HourOfDay11,  HourOfDay12, HourOfDay13,  HourOfDay14,  HourOfDay15, HourOfDay16,  HourOfDay17,  HourOfDay18, HourOfDay19,  HourOfDay20,  HourOfDay21, HourOfDay22,  HourOfDay23, DayOfMonth02, DayOfMonth03, DayOfMonth04, DayOfMonth05, DayOfMonth06, DayOfMonth07, DayOfMonth08, DayOfMonth09, DayOfMonth10, DayOfMonth11, DayOfMonth12, DayOfMonth13, DayOfMonth14, DayOfMonth15, DayOfMonth16, DayOfMonth17, DayOfMonth18, DayOfMonth19, DayOfMonth20, DayOfMonth21, DayOfMonth22, DayOfMonth23, DayOfMonth24, DayOfMonth25, DayOfMonth26, DayOfMonth27, DayOfMonth28, DayOfMonth29, DayOfMonth30, DayOfMonth31 ReportCategoryExtract, ServerSQLODR3, ServerSQLODR6

```{r Obj1 Final Model Selection, cache=TRUE, echo=FALSE, warning=FALSE}
...
```{r Obj1 Assumptions, fig.height=3, fig.width=3}
# QQ Plot shows a slight departure from normality:
ols_plot_resid_qq(final.model)
# Cook's D shows no major leverage points (.003)
ols_plot_cooksd_chart(final.model)
# There are a few outliers with leverage in the sample, but not so much in the full data:
ols_plot_resid_lev(final.model)
# Histogram shows normality with a bit of right skew
ols_plot_resid_hist(final.model)
# Ideally we'd like to see a totally random cloud here
ols_plot_resid_fit(final.model)
# These outliers are impacting predictions fairly significantly
ols_plot_obs_fit(final.model)
...
### Conclusion
In conclusion, the mulit-linear model with 5 predictors leveraging a logged response gave the best MSE with avoidance of over-fitting. We used a train and test split to validate the MSE across many method and reach this conclusion.  
...
```{r Performance setup, cache=TRUE}
...
# Objective 2:
Our question of interest from Gilsbar is which server, and what time of day yields the best performance to schedule new reports? 
...
```{r UnloggedResponseResidual, cache=TRUE}
...
```{r loggedResponsesummary, cache=TRUE}
...
```{r loggedResponseResidual, cache=TRUE}
