...
reports$TimeBlock <- case_when(reports$HourOfDay %in% 
...
reports$TimeBlock2 <- case_when(reports$HourOfDay %in% 
                              c("00","01","02","03","04","05") ~ "Maintenance",
                            reports$HourOfDay %in% 
...
fit <- lm(ReportBuildTime~GroupCount+AgentCount+NumDays+ReportCategory , data=reports)

#reports$NumDays <- ifelse(is.na(reports$NumDays),43574, reports$NumDays)
#reports$NumDays <- ifelse(reports$NumDays == 0,43574, reports$NumDays)

max(reports$NumDays)
max(reports$GroupCount)
...
log(reports$ReportDeliveryTime)
log(reports$ReportBuildTime)
log(reports$GroupCount)
log(reports$NumDays)

#reports2<- reports %>% filter(ReportBuildTime < 7200) # Reports running over 2 hours are anomoly but possible. 4 hours is hard cutoff for build time.


fit <- lm(log(ReportBuildTime)~ReportCategory + GroupCount + GroupCount*ReportCategory + HourOfDay, data=reports)
...
trainIndices = sample(1:dim(reports)[1],round(.7 * dim(reports)[1]))
  train = reports[trainIndices,]
  test = reports[-trainIndices,]
  prd <- predict(fit3, interval="predict",newdata = test)
RMSE <- sqrt(mean((prd[,1] - test$ReportDeliveryTime)^2))
RMSE
plot(log(test$ReportBuildTime), prd[,1])



fit <-lm(log(ReportBuildTime)~ReportCategory+first+six+ReportCategory*six+ReportCategory*first+first*six+six*first*ReportCategory, data=reports)
summary(fit)

tic('fit')
#fit <-lm(ReportBuildTime~ReportCategory, data=reports) # r2 = 29.07, , 7055 RMSE
fit <-lm(log(ReportBuildTime)~as.factor(ReportId), data=reports) # r2 = 79.47, pred time = ~240 sec, 7260 RMSE
summary(fit)

tic('fit')
fit2 <- lm(log(ReportDeliveryTime)~as.factor(ReportId), data=reports)
fit3 <- lm(log(ReportBuildTime)~as.factor(ReportId), data=reports)

summary(fit2)

#fit <-lm(log(ReportBuildTime)~as.factor(ReportId)+GroupCount, data=reports) # r2 = 79.71 ~209 sec
#fit <-lm(log(ReportBuildTime)~as.factor(ReportId)+HourOfDay, data=reports) # r2 = 79.66 ~226 sec
#fit <-lm(log(ReportBuildTime)~as.factor(ReportId)+DayOfMonth+HourOfDay+GroupCount+RptFrmt+Server, data=reports) # r2 = 80.65 ~247 sec
#fit <-lm(log(ReportBuildTime)~as.factor(ReportId)first+six, data=reports) # r2 = 79.47 ~241 sec

#summary(fit)
toc()


fit2 <-lm(log(ReportDeliveryTime)~ReportCategory+TimeBlock+ReportCategory*TimeBlock, data=reports)
summary(fit2)
...
reports2 <- reports %>% filter(HourOfDay %in% c("06","07","08","09","10","11","12","13","14","15","16","17","18"))
reports2 <- reports2 %>% filter(DayOfWeek %in% c("Monday","Tuesday","Wednesday","Thursday","Friday"))

reports$first <- ifelse(reports$DayOfMonth == "01", 1, 0)
reports$six <- ifelse(reports$HourOfDay == "06", 1, 0)


reports$NumParams <- ifelse(is.na(reports$XMLResponseStringClean) | reports$XMLResponseStringClean == '', 1, str_count(reports$XMLResponseStringClean, '</'))


r <- reports %>% filter(ReportCategory == 'Professional Liability')
...
fit <- lm(log(ReportDeliveryTime)~GroupCount+NumParams+HourOfDay+DayOfMonth, data=r)
summary(fit)

r <- reports %>% filter(ReportCategory == 'Finance Commission')

r$clients <- sapply(r$XMLResponseStringClean, function(x) 
  ifelse(x %like% '<ClientCode>', str_replace(x, '.ClientCode>(\\d+)</.*', '\\1'), NA))


t <- table(log(test$ReportDeliveryTime),prd[1])
plot(log(test$ReportDeliveryTime), prd[,1])
