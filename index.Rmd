...
```{r Dustins MLR options, cache=TRUE}
...
# Objective 1:
Using known parameters of a desired report, can we predict how long it will take to run a particular report? 
...
Given some additional knowledge of the dataset, we learned that the ReportID (Specific Report) may related to the ReportBuildTime.  To explore this, we converted the ReportID to factors and conducted simple linear regression, giving us an R^2 of 0.79 which is very good, however the ReportID has over 1500 factor levels which make the explanation difficult, so we have to make other fittings and temporarily ignore ReportID.
Some additional pre-knowledge of the dataset make us filter the dataset further: Only the 3 “working” servers are included and only the TestProdIndicator equal to”P” are preserved. Because the amount of data are very large and we don’t find a dependency of ReportID with ReportDeliveryTime, we choose a single type of report “ReportID=93”, which is roughly 1/10 of the overall dataset as a representative subset and use it for later analysis.
...
reports_serv_samp <- reports_serv %>% filter(ReportId == "93") 
