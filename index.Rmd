...
2. What are the best times of day, week, or month to run a report?
3. What are the worst times of day, week, or month to run a report?
...
* NumberOfQueuedReports - How many reports are in the queue and/or processing when the report batch begins rendering
...
Finance
"Deposit","CommCompare","Credit Card Process","CheckRecon","Commissions","CRMOnlineFinance","CommCnt","Check Register","FinancePremium"
...
Billing
"Billing", "Premium","Coverage","Refunds"


### ReportGroup Crosswalk:
When = 'CommissionsTFB' override 'renewal' with 'Commissions'
...
reports$CurrencyCode <- sapply(reports$XMLResponseStringClean, function(x) 
  ifelse(x %like% '<CurrencyCode>', xml_find_all(read_xml(x), './/CurrencyCode') %>% xml_text(), NA))
...
cat <- reports %>% select(ReportGroupDescription) %>% arrange() %>% unique()

write.csv(cat, file='cat2.csv', row.names = F)
